                        ...
                        <div class="col-md-12">
                            <div class="alert alert-info">
                                ...
                                <div class="row">
                                    <div class="col-md-3">
                                        <strong>Paid:</strong>
                                        {{ $courseEnrollments->where('payment_status', 'paid')->count() }}
                                    </div>
                                    <div class="col-md-3">
                                        <strong>Partial:</strong>
                                        {{ $courseEnrollments->where('payment_status', 'partial')->count() }}
                                    </div>
                                    <div class="col-md-3">
                                        <strong>Pending:</strong>
                                        {{ $courseEnrollments->where('payment_status', 'pending')->count() }}
                                    </div>
                                    <div class="col-md-3">
                                        <strong>Overdue:</strong>
                                        {{ $courseEnrollments->where('payment_status', 'pending')->where('payment_due_date', '<', now())->count() }}
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-3">
                                <input type="text" name="search" class="form-control"
                                    placeholder="Search by student or course name..." value="{{ request('search') }}">
                            </div>
                            <div class="col-md-2">
                            ...
                            <div class="col-md-2">
                                <select name="payment_status" class="form-control">
                                    ...
                                    <option value="overdue"
                                        {{ request('payment_status') === 'overdue' ? 'selected' : '' }}>Overdue</option>
                                </select>
                            </div>
                            <div class="col-md-2">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <small
                                            class="text-muted">{{ $enrollment->course->teacher->specialization }}</small>
                                        @else
                                    ...
                                    <td>
                                        ...
                                        <small
                                            class="text-muted">${{ number_format($enrollment->total_fee, 2) }}</small>
                                        @break
                                        @case('partial')
                                        <span class="badge bg-warning text-dark">Partial</span><br>
                                        <small class="text-muted">${{ number_format($enrollment->amount_paid, 2) }} /
                                            ${{ number_format($enrollment->total_fee, 2) }}</small>
                                        @break
                                        @case('pending')
                                        ...
                                        @endif
                                        <br>
                                        <small class="text-muted">${{ number_format($enrollment->amount_paid ?? 0, 2) }}
                                            / ${{ number_format($enrollment->total_fee, 2) }}</small>
                                        @if($enrollment->payment_due_date)
                                        <br><small class="text-muted">Due:
                                            {{ $enrollment->payment_due_date->format('M j, Y') }}</small>
                                        @endif
                                        @break
                                        @default
                                        <span
                                            class="badge bg-secondary">{{ ucfirst($enrollment->payment_status ?? 'Unknown') }}</span><br>
                                        <small class="text-muted">${{ number_format($enrollment->amount_paid ?? 0, 2) }}
                                            / ${{ number_format($enrollment->total_fee, 2) }}</small>
                                        @endswitch
                                        @else
                                    ...
                                    <td>
                                        ...
                                        <small
                                            class="text-muted">{{ $enrollment->approved_at->format('g:i A') }}</small>
                                        @else
                                    ...
                                    <td>
                                        <div class="btn-group" role="group">
                                            ...
                                            <!-- Approve Button -->
                                            <form method="POST"
                                                action="{{ route('employee.course-enrollments.update-status', $enrollment) }}"
                                                class="d-inline">
                                                @csrf
                                                @method('PATCH')
